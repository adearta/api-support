...
class SchoolParticipantAkbarController extends Controller
{
    ...
    public function updateStatus(Request $request, $webinarId)
    {
        $needValidate = array(
            'webinar_id'    => $webinarId,
            'school_id'     => $request->school_id,
        ...
        $validation = Validator::make($needValidate, [
            'webinar_id'    => 'required|numeric|exists:' . $this->tbWebinar . ',id',
            'school_id'     => 'required|numeric|exists:pgsql2.' . $this->tbSchool . ',id',
            'student.*'     => 'required|numeric|exists:pgsql2.' . $this->tbUserEdu . ',start_year'
        ]);
        if ($validation->fails()) {
            return $this->makeJSONResponse(['message' => $validation->errors()->first()], 400);
        } else {
            ...
            $schParticipantValidation = DB::table($this->tbSchoolParticipant)
                ...
                ->get();
            if ($schParticipantValidation[0]->status >= 1) {
                $schoolParticipant = DB::select('select schedule, id from ' . $this->tbSchoolParticipant . " where school_id = " . $request->school_id . " and webinar_id = " . $webinarId);
                // var_dump($schoolParticipant);
                $date = date("Y-m-d");
                $status = 4;
                if ($schoolParticipant[0]->schedule >= $date) {
                    $this->updateStatusParticipant($webinarId, $request->school_id, $status, $date);
                    ...
                    $newparticipants = 0;
                    $total = count($participant);
                    for ($s = 0; $s < count($request->student); $s++) {
                        $student = DB::connection('pgsql2')->table($this->tbUserEdu, 'edu')
                            ->leftJoin($this->tbStudent . ' as student', 'edu.nim', '=', 'student.nim')
                            ->where('edu.school_id', '=', $request->school_id)
                            ->where('edu.start_year', '=', $request->student[$s])
                            ->where('edu.verified', '=', true)
                            ->where('student.id', '!=', null)
                            ->get();
                        $studentArr[$s] = $student[0];
                    }
                    echo count($studentArr);
                    // $newparticipants = count($studentArr[$s]);
                    $countparticipants = $total + $newparticipants;

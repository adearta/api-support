...
class SchoolParticipantAkbarController extends Controller
{
    ...
    public function sendInvitation($webinarId, Request $request)
    {
        /* the status of school
            ...
            5.-> finished webinar
        */
        $request->merge(['webinar_id' => $webinarId]);
        $validation = Validator::make($request->all(), [
            'school_id'     => 'required|numeric|exists:pgsql2.' . $this->tbSchool . ',id',
        ]);
        ...
        if ($validation->fails()) {
            ...
            $secondValidation = Validator::make($request->all(), [
                'webinar_id'    => 'required|numeric|exists:' . $this->tbSchoolParticipant . ',webinar_id,school_id,' . $request->school_id,
                'student'       => 'required|array',
                'student.*'     => 'required|numeric|exists:pgsql2.' . $this->tbUserEdu . ',start_year,school_id,' . $request->school_id,
            ]);

            if ($secondValidation->fails()) {
                return $this->makeJSONResponse(['message' => $secondValidation->errors()->first()], 400);
            } else {
                $response = DB::transaction(function () use ($request) {
                    ...
                    $webinar = WebinarAkbarModel::findOrFail($request->webinar_id);

                    if ($schoolParticipant[0]->status == 3 || $schoolParticipant[0]->status == 4) {
                        if ($webinar->event_date >= date("Y-m-d")) {

                            $participant = StudentParticipantAkbarModel::where('webinar_id', '=', $request->webinar_id)->count();

                            $years = "ARRAY[";
                            foreach ($request->student as $index => $year) {
                                if ($index == 0) {
                                    $years .= $year;
                                } else {
                                    $years .= "," . $year;
                            ...
                            if ($countparticipants <= 500) {
                                ...
                                $this->updateStatusParticipant($request->webinar_id, $request->school_id, 4);
                                //success add data
